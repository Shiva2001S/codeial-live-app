...
class Post extends Component {
    constructor(props) {
      ...
      handlePostLike = () => {
        ...
        this.props.dispatch(addLike(post.id, 'Post', user.userId))
      }
  render() {
    const { post, user } = this.props;
    const { comment } = this.state;

    const isPostLikedByUser = post.likes && post.likes.includes(user.userId);
            ...
            <div className="post-header">
              ...
              <div className="post-actions">
                <button className="post-like no-btn" onClick={this.handlePostLike}>
                  ...
                  {isPostLikedByUser ?
                  (<img
                    src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRZi7FOJUGoQ3HCV3QzFLVYF89Qape9qv5pa0vqPGOU&s"
                    alt="likes-icon"
                  />)
                  :
                  (<img
                    src="https://img.freepik.com/premium-vector/hand-drawn-realistic-3d-heart-decorative-spring-romantic-icon-love-symbol-happy-valentines-day-icon-white-heart-abstract-vector-illustration-isolated-white-background_245580-1585.jpg"
                    alt="likes-icon"
                  />)
                  }
              ...
              <div className="post-comments-list">
                ...
                <Comment comment={comment} key={comment._id} postId={post.id} />
                ))}
